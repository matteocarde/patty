...
void CSolver::set_var_value_BCP(int v, int value) {
  ...
  for (int xxx = 0; xxx<watchs.size(); xxx++)
  {
    vector <CLitPoolElement *>::iterator itr = watchs.begin();
	for(int yyy = 0; yyy<xxx; yyy++)
		itr++;
	ClauseIdx cl_idx;
    ...
    while (true) {
      ...
      *itr = watchs.back();  // copy the last element in it's place
	  ...
	  --xxx;                 // do this so with don't skip one during traversal
      break;
    }
  }
}
