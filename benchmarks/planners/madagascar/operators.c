...
int actionCmp(const void *p1, const void *p2)
{
  const action *a1 = p1;
  const action *a2 = p2;
  int v;
...
void mergecontras() {
  int l0,l,l2,cnt;
  int i;
  converse = (int *)malloc(nOfAtoms * nOfAtoms);

  for(i=0;i<nOfAtoms;i++) converse[i] = -1;
  ...
  for(l0=0;l0<nOfAtoms;l0++) {
	...
	//	}
	if((converse[feVAR(l2)] == -1) ||
	   ((converse[feVAR(l2)] >> 1) > feVAR(l))) {
	  converse[feVAR(l2)] = ((l&1) == (l2&1)) + (feVAR(l) << 1);
	}
	cnt += 1;
      }
    }
  ...
  if(flagShowInput) {
    ...
    for(i=0;i<nOfAtoms;i++) { if(converse[i] != -1) { printatomi(i); printf(" will be replaced by "); if(converse[i] & 1) printf("NOT "); printatomi(converse[i] >> 1); printf("\n"); } }
  }
  for(i=0;i<nOfActions;i++) renameactionL(&(actions[i]),converse);
  for(i=0;i<nOfAtoms;i++) {
    if(converse[i] != -1) { deletetwolits(fePLIT(i)); deletetwolits(feNLIT(i)); }
    eliminatefromtwolits(twolits[fePLIT(i)],converse);
    eliminatefromtwolits(twolits[feNLIT(i)],converse);
  }
  renamefmaL(goal,converse);
}
