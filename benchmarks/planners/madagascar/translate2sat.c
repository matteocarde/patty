...
void runalgorithmC(void);
void initclauseset(void);
...
/* Detect interference when other action's effect may make a formula
   in the action true or false.
   If the polarity flag is set, then consider both making true and false.
   Otherwise only consider making false.
*/
  ...
  int i,j;
    ...
    */
	...
	while(*ls != -1) {
	  ...
	  j = 0;
	  int variable_id = *ls / 2;
	  while(j < evarcnt && evars[j] != variable_id) {
	    j = j+1;
	  }
...
void encoding() {
  ...
  /* There must be at least one state variable. WHY? */
...
#define RANDOMORDER
    ...
    /* Require at least one action for every step where goals do not hold? */
    
    if(flagAtLeastOneAction) {
      int aux = allocAUX(1);
      fmalist *allActions;
      int i;
      allActions = NULL;
      for(i=0;i<nOfActions;i++) allActions = fmacons(fmaOP(i),allActions);
      addformula(transition,Fdisj2(makeVARfma(goal),Fatom(AUX(aux))));
      addformula(transition,Fimpl(Fatom(AUX(aux)),Fdisj(allActions)));
    }
...
void outputDIMACS() {
  ...
  for(i=firstTimePoint;i<=lastTimePoint;i+=outputTimeStep) { /* Write files for different lengths. */
    ...
    /* Output int to symbol mapping as DIMACS comments. */
    fprintf(F,"c format for actions: index time name\n");
    for(j=0;j<i;j++) { /* Time points for actions */
      bias = j*varsPerTime;
      for(k=0;k<nOfActions;k++) {
	fprintf(F,"c action %i %i ",k+bias+nOfAtoms+1,j);
	fprintactionname(F,k);
	fprintf(F,"\n");
      }
    }
    fprintf(F,"c format for variables: index time name\n");
    for(j=0;j<=i;j++) { /* Time points for state variables */
      bias = j*varsPerTime;
      for(k=0;k<nOfAtoms;k++) {
	if(converse[k] != -1) {
	  fprintf(F,"c variable %i ",k+bias+1);
	  fprintatomi(F,k);
	  fprintf(F," is represented by ");
	  if(converse[k]&1) fprintf(F,"NOT ");
	  fprintatomi(F,feVAR(converse[k]));
	} else {
	  fprintf(F,"c var %i %i ",k+bias+1,j);
	  fprintatomi(F,k);
	}
	fprintf(F,"\n");
      }
