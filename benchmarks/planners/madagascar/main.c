...
#define STATMALLOCS 10000

#ifdef SHOWMALLOCSTATS
long statmallocs[STATMALLOCS];

void initstatmalloc() {
  int i;
  allocatedbyFE = 0.0;
  for(i=0;i<STATMALLOCS;i++) statmallocs[i] = 0;
}
...
#include "zPRINTDATE"
  ...
  if(argc == 1) {
    ...
    printf("-1     Force at least one action every step\n");
  ...
  for(i=1;i<argc;i++) {
	...
	case 3: planSemantics = EStepOgata; flagCEvariables = 0; break;
	}
	i++;
	printf(" -P %i",j);
	break;
	...
	printf(" -E");
	break;
      case '1': /* At least one action every step. */
	flagAtLeastOneAction = 1;
	printf(" -1");
	break;
      default:
	printf("ignoring %s\n",argv[i]);
