...
def main():
    ...
    for plannersDict in plannersDicts:
        ...
        for i, p in enumerate(plots):
            ...
            for planner in plannersDict["planners"]:
                pInfo = JAIR_PLANNERS[planner]
                print(pInfo["name"][1:-1])
                ax.plot(cactusByPlanner[planner], Y / p["scalingFactor"], label=pInfo["name"],
                        linestyle=pInfo["style"], linewidth=2)

            ax.legend(loc="upper left", fontsize="10")

            filename = f"{exp}-{p['key']}-{plannersDict['name']}.pdf"
