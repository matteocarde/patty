...
def main():
    ...
    joinWith = [
        ...
        ("2024-10-07-AIJ-FINAL-v2", ["ENHSP-SAT-AIBR", "ENHSP-SAT-HADD", "ENHSP-SAT-HMRP", "METRIC-FF", "NFD"])
    ]
    ...
    for domain, domainDict in dOrig.items():
        if domain not in PUSHING_DOMAINS_TABLE2:
            continue
        for planner, plannerDict in domainDict.items():
            ...
            for problem, problemList in plannerDict.items():
                if problem not in PUSHING_DOMAINS_TABLE2[domain]["instances"]:
                    continue
