...
from pysmt.shortcuts import Plus, Equals, LE, GT, GE
...
GREATER_EQUAL_CACHE: Dict[Tuple[SMTExpression, SMTExpression], SMTExpression] = dict()
...
class GreaterEqualExpression(BinaryExpression):
    ...
    def simplify(cls, *xs):
        ...
        if (lhs, rhs) in GREATER_EQUAL_CACHE:
            return GREATER_EQUAL_CACHE[lhs, rhs]
        GREATER_EQUAL_CACHE[lhs, rhs] = cls(lhs, rhs)
        return GREATER_EQUAL_CACHE[lhs, rhs]
    ...
    def getExpression(self, memodict=dict()) -> FNode:
        ...
        expr = GE(self.lhs.getExpression(memodict=memodict), self.rhs.getExpression(memodict=memodict))
        memodict[self] = expr
        return expr
