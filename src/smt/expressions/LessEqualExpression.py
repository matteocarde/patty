...
LESS_EQUAL_CACHE: Dict[Tuple[SMTExpression, SMTExpression], SMTExpression] = dict()
...
class LessEqualExpression(BinaryExpression):
    ...
    def simplify(cls, *xs):
        ...
        if (lhs, rhs) in LESS_EQUAL_CACHE:
            return LESS_EQUAL_CACHE[lhs, rhs]
        LESS_EQUAL_CACHE[lhs, rhs] = cls(lhs, rhs)
        return LESS_EQUAL_CACHE[lhs, rhs]
    ...
    def getExpression(self, memodict=dict()) -> FNode:
        ...
        expr = LE(self.lhs.getExpression(memodict=memodict), self.rhs.getExpression(memodict=memodict))
        memodict[self] = expr
        return expr
